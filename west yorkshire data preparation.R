...
# reading the stop-and-search dataset for west yorkshire (3 years)
temp_ss <- list.files(path = paste0(getwd(), "/west yorkshire 3yrs data"), pattern = "*search.csv")
temp_ss <- paste0(getwd(), "/west yorkshire 3yrs data/", temp_ss)
...
# reading the street crime dataset for west yorkshire (3 years)
temp_st <- list.files(path = paste0(getwd(), "/west yorkshire 3yrs data"), pattern = "*street.csv")
temp_st <- paste0(getwd(), "/west yorkshire 3yrs data/", temp_st)
...
################# DATA CLEANING #################
# removing na column and rows without lat/long and crime ID in wesryorkshire street data
df_st <- df_street %>%
...
# saveRDS(df_st, "westyorkshire_st.rds")
...
# saveRDS(df_ss, "westyorkshire_ss.rds")
